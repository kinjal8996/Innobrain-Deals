...
class AdminBannerController extends Controller
{
    ...
    public function bannerdetailupdate($id, Request $request)
    {
        ...
        $bannerdetail->description = $request->input('description');
        if ($request->hasFile('image')) {
            if (!empty($bannerdetail->image) && file_exists(public_path('BannerImage/' . $bannerdetail->image))) {
                unlink(public_path('BannerImage/' . $bannerdetail->image));
            }

            $file = $request->file('image');
            $fileName = time() . "img." . $file->getClientOriginalExtension();
            $file->move(public_path('BannerImage'), $fileName);
            $bannerdetail->image = $fileName;
        }
